...
import {mockSendEmail, mockgetAdminDetails} from '../utils/mock_services';
...
test('SUBSCRIPTION NEW PAYMENT', async () => {
  ...
  expect(subscription_old_payment.no_of_orders_consumed).toBe(20);

  const subscription_new_payment: ISubscriptionPayment = (
    await DB.read.from('subscription_payment').where({
      subscription_id: subscription.id,
      id: 2,
    })
  )[0];
  expect(subscription_new_payment.status).toBe(
    SubscriptionPaymentStatus.SUCCESS
  );
  expect(subscription_new_payment.no_of_orders_bought).toBe(20);
  expect(subscription_new_payment.no_of_grace_period_orders_allotted).toBe(6);
  expect(subscription_new_payment.no_of_orders_consumed).toBeNull();

  const updated_restaurant_with_new_benifits: IRestaurant_Basic = (
    await DB.read.from('restaurant').where({id: restaurant_id})
  )[0];
  expect(
    updated_restaurant_with_new_benifits.subscription_end_time
  ).not.toBeNull();
  expect(
    updated_restaurant_with_new_benifits.subscription_remaining_orders
  ).toBe(17); // 20 - 3 orders consumed in last grace period  = 17
  expect(
    updated_restaurant_with_new_benifits.subscription_grace_period_remaining_orders
  ).toBe(6);
  expect(updated_restaurant_with_new_benifits.subscription_id).toBe(
    subscription.id
  );
  ...
  expect(updated_subsription.current_cycle).toBe(
  ...
  const filter_plan_fail_response = await request(server)
    .post('/food/admin/subscription/payment/filter')
  ...
  expect(filter_plan_success_response.body.result.total_records).toEqual(2);
