...
test('CHANGE SUBSCRIPTION TEST', async () => {
  const free_plan = await createTestPlan({
    id: 'RES_b5372ad4-0a69-4dc1-a936-26393cd4b133',
    name: 'Free Plan',
    type: PlanType.FREE,
  ...
  const paid_plan = await createTestPlan({
  ...
  const mock_create_external_subscription =
    mockCreateSubscriptionAtExternalService(paid_plan.id);
