...
describe('Sort By Delivery time', () => {
  test('In desc order', async () => {
    /***
     *  '97c6fdbc-8df6-4541-9d3f-f9e5ba4c0242' 6 mins online - 1 rank
     *  'b0909e52-a731-4665-a791-ee6479008805' 12 mins offline - 3 rank
     *  '609a460e-6316-417e-9e87-836bfbcded0f' 17 mins no subscription - will not come in response
     *  '77e53c1f-6e9e-4724-9ba7-92edc69cff6b' 23 mins online - 2 rank
     */
    ...
    const page_0_response = await request(server)
      ...
      .send({
        filter: {
          sort_by: RestaurantSortBy.DELIVERY_TIME,
    ...
    expect(
      page_0_response.body.result.restaurants[0].delivery_time_in_seconds
    ).toBeLessThan(
      page_1_response.body.result.restaurants[0].delivery_time_in_seconds
    );
    ...
    if (
      ...
      expect(
        ...
        page_2_response.body.result.restaurants[0].delivery_time_in_seconds
      );
    }
  });
});
