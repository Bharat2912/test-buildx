 ...
 * After Some Time Customer Cancel Order With Reason
 * Refund Should Not Given To Customer
 */
...
describe('Test Case Scenerio For Vendor Reject Order', () => {
  ...
  describe('Cart', () => {
    test('Customer Added Items In Cart : pod_allowed should be FALSE', async () => {
      mockCartServiceabilityWithValidResponse();
    ...
    test('POD Order will not be placed as cart is not alloed for pod', async () => {
      mockPostServiceableAddress();
      mockGetCustomerDetails();
      mockGetTransactionToken();
    ...
    test('Customer Added Items In Cart  : pod_allowed should be TRUE', async () => {
      mockCartServiceabilityWithValidResponse();
      const response = await request(server)
        .put('/food/cart')
        .set('Authorization', `Bearer ${customer_token}`)
    ...
    test('After All Valid Items Cart Will Be Created ', async () => {
      ...
      Order_Details = placeOrderResponse.body.result;
    });
  });
  ...
  describe('Vendor Recived And Accept Order', () => {
  ...
  describe('Customer Cancelled The Order After Some Time With Cancellation Reason', () => {
    test('Customer | Cancel Order', async () => {
      mockCancelDeliverySuccess();
      const response = await request(server)
    ...
    test('Customer | Check Order Refund Status And Order Status | Refund Status Must Be Pending And Order Status Must Be Cancelled', async () => {
      const response = await request(server)
        .get(`/food/order/${OrderId}`)
        .set('Authorization', `Bearer ${customer_token}`);
      expect(response.body.status).toBe(true);
    ...
    test('VENDOR | Check For Order Status | Order Status Must Be Cancelled And Order Acceptence Status Must Be Accepted', async () => {
      const response = await request(server)
        .get(`/food/order/${OrderId}`)
        .set('Authorization', `Bearer ${customer_token}`);
      expect(response.body.status).toBe(true);
      ...
      expect(response.body.result.records[0].delivery_status).toBe('cancelled');
      expect(response.body.result.records[0].order_status_code).toBe(113);
    });
    test('Database Check For Order Details |  Order Status Must Be Cancelled And Refund Status Must Be Pending', async () => {
      ...
      expect(read_order_details[0].delivery_details).not.toBe({});
  ...
  describe('Refund Should Not Given to Customer', () => {
    test('ADMIN | Cannot Initiate Refund', async () => {
      mockgetAdminDetails();
      const response = await request(server)
        ...
        .send({
          ...
          refund_settlement_note_to_vendor: 'order was cancelled by customer',
          refund_settlement_note_to_customer: 'order was cancelled by customer',
        });
      expect(response.body.status).toBe(false);
      expect(response.statusCode).toBe(400);
      expect(response.body.errors[0].message).toBe(
        "refund status on order 1 is not in 'approval pending' state"
      );
    });
    test('ADMIN | To Check Order Details | Order Status Must Be Cancelled and Order Acceptence Status Must Be Accepted', async () => {
      mockgetAdminDetails();
      const response = await request(server)
        .get(`/food/admin/order/${OrderId}`)
        .set('Authorization', `Bearer ${admin_token}`);
      ...
      ).toStrictEqual('selected wrong delivery location');
      expect(response.body.result.records[0].vendor_accepted_time).not.toBe(
        null
      );
      expect(response.body.result.records[0].cancellation_time).not.toBe(null);
    ...
    test('Vendor | Check for Order Status | Order Status Must Be Cancelled With Cancellation Accepted', async () => {
      const response = await request(server)
    ...
    test('Database Check For Order Details |  Order Status Must Be Cancelled', async () => {
      ...
      expect(read_order_details[0].delivery_details).toMatchObject({
        ...
        eta_when_vendor_accepted: {
          rider_from_vendor_to_customer_eta: 5,
          rider_to_vendor_eta: 5,
          preparation_time: 10,
        },
      });
      expect(read_order_details[0].order_status).toBe('cancelled');
