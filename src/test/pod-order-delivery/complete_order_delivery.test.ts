...
describe('Full POD-Order Flow Test Cases', () => {
  let OrderId: string;
  ...
  describe('Vendor Recived Order', () => {
    ...
    test('VENDOR | Check For Order Status | Delivery Status Must Be Arrived', async () => {
      const response = await request(server)
        .get(`/food/order/${OrderId}`)
        .set('Authorization', `Bearer ${customer_token}`);
      expect(response.body.status).toBe(true);
    ...
    test('CUSTOMER | Check for Order Status | Delivery Status must be DISPATCHED ', async () => {
      const response = await request(server)
        .get(`/food/order/${OrderId}`)
        .set('Authorization', `Bearer ${customer_token}`);
      expect(response.body.status).toBe(true);
      ...
      expect(response.body.result.records[0].order_status_code).toBe(107);
    });
    test('Database Check For Order Details', async () => {
      const read_order_details = await DB.read('order').where({
        id: OrderId,
    ...
    test('VENDOR | Mark Order As Ready', async () => {
      ...
      expect(orderAccept.body.result.order_status).toBe('placed');
      expect(orderAccept.body.result.order_acceptance_status).toBe('accepted');
      expect(orderAccept.body.result.delivery_status).toBe('dispatched');
    });
    test('Database Check For Order Details', async () => {
      const read_order_details = await DB.read('order').where({
        id: OrderId,
