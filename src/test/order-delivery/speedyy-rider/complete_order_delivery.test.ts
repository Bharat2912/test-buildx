...
beforeAll(async () => {
  ...
  shadowfax_token = process.env.RIDER_APP_CALLBACK_TOKEN || '';
    ...
    test('Driver Allotted For Order', async () => {
      ...
      const response = await request(server)
        ...
        .send({
          ...
          rider_id: '',
          rider_image_url: '',
          rider_latitude: 12.975777,
          rider_longitude: 77.59818,
          rider_name: 'Speedyy',
        });
    ...
    test('Driver Arrived At Restaurant | Mark Delivery Status As Arrived', async () => {
      ...
      const response = await request(server)
        ...
        .send({
          ...
          rider_id: '',
          rider_image_url: '',
          rider_latitude: 12.97617,
          rider_longitude: 77.59827,
          rider_name: 'Ankita',
        });
      expect(response.status).toBe(200);
      expect(response.text).toBe('OK');
    ...
    test('Driver Has Deliverd Order | Mark Delivery Status As DELIVERED', async () => {
      ...
      const response = await request(server)
        ...
        .send({
          ...
          rider_id: '',
          rider_image_url: '',
          rider_latitude: 12.977962,
          rider_longitude: 77.59524,
          rider_name: 'Speedyy',
        });
      expect(response.status).toBe(200);
      expect(response.text).toBe('OK');
