...
describe('Petpooja Push Menu Items (Menu items) Test Cases', () => {
  ...
  test('Deleting existing addons and addon group', async () => {
    ...
    expect(read_menu_item_addon.length).toBe(5);
  ...
  test('Adding new addon in existing addon group', async () => {
    PETPOOJA_TEST_MENU_CLONE.addongroups = [
      {
        addongroupid: '8382',
        addongroup_rank: '1',
    ...
    //read  addons
    const read_addons = await DB.read('addon').select('*');
    expect(read_addons.length).toBe(13);
    expect(read_addons).toStrictEqual([
      {
        addon_group_id: 1,
        cgst_rate: 0,
    ...
    expect(read_menu_item_addon.length).toBe(8);
    expect(read_menu_item_addon).toStrictEqual([
      {
        addon_id: 11,
        menu_item_id: 2,
